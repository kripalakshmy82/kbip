...
function validateForm(form,baseUrl,fun) 
{
	...
	// if (document.getElementById('acaptionmal').value.trim()=="" && error==false)
	// {
	// 	loadErrorModal('Please provide announcement title in malayalam','acaptionmal');
	// 	document.getElementById('acaptionmal').focus();
	// 	error=true;
	// 	return false;
	// }
...
function deletePgallery(baseUrl,pgallery_id,page)
{
	...
	if(pgallery_id!="")
	{
		...
		loadConfirmModal("Do you want to Delete this announcement");
		$("#yesBtn").unbind().click(function()
		{
			...
			xmlhttp.open("GET",baseUrl+"announcement/delete_chosenannouncement/"+pgallery_id,true);
			xmlhttp.send();
		});
	}
} 
function changestatus_announcement(baseUrl,announcement_id,status)
{
	...
	if(announcement_id!="")
	{
		...
		if(status==3)
		{
			loadConfirmModal("Do you want to add this Announcement to archive list?");
		}
		if(status==2)
		{
			loadConfirmModal("Do you want to Remove this Announcement ?");
		}
		$("#yesBtn").unbind().click(function()
		{
			//alert(baseUrl+"ajax_functions/delete_rtioffice/"+office_id);
...
function changestatus_announcement_detail(baseUrl,announcement_id,status)
{
	...
	if(announcement_id!="")
	{
		...
		if(status==2)
		{
			loadConfirmModal("Do you want to Remove this Annoucement?");
		}
