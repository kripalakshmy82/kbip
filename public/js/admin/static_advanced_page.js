...
function validateStatic(form,baseUrl,fun)
{
	...
	if(form.caption.value.trim()==0 && error==false)
	{
		error=true;
		loadErrorModal('Please enter the caption','caption');
	}
	...
	if(error == false)
	{
		if(fun=='add')
		{
			form.action=baseUrl+"static_advanced_page/insert_page/";
		...
		if(fun=='update')
		{
			form.action=baseUrl+"static_advanced_page/update_page/";
		...
		$('#yesBtn').click(function()
		{
			...
			{	
				...
				var rtext = xmlhttp.responseText;
					...
					if(rtext=='1')
					{
						 window.location.href=baseUrl+'static_advanced_page/static_list_view/'+page;
					}
								
				}
			}
			xmlhttp.open("GET",baseUrl+"static_advanced_page/change_status_static/"+cl_id+"/"+status,true);
			xmlhttp.send();
		});
	}
}
...
function deleteStatic(baseUrl,cl_id,page)
{
	...
	if(cl_id!="")
	{
		...
		$('#yesBtn').click(function()
		{
			...
			//alert(baseUrl+"static_advanced_page/delete_static/"+cl_id);
			...
			xmlhttp.open("GET",baseUrl+"static_advanced_page/delete_static/"+cl_id,true);
			xmlhttp.send();
		});
	}
}
...
function Delete_Photo(baseUrl,id,file)
	...
	$('#yesBtn').click(function()
		{
			window.location.href=baseUrl+'static_advanced_page/delete_photo/'+id+'/'+file;
		});
	}
...
{
	var error=false;
	if((form.caption.value.trim()=='' && form.heading.value.trim()=='') && error==false)
	{
		error=true;
		loadErrorModal('Please enter caption or heading','heading');
		//alert("Please enter caption or heading");
		//document.getElementById('caption').focus();
	}
	if(error == false)
	{
	 form.action=baseurl+"static_advanced_page/static_list_view/";
	 form.submit();
	}
}
